...
func (db *FavoritesRepository) DeleteFavoritesByMovieId(userId int, movieId int) error {
	tx, err := db.Database.Begin()
	if err != nil {
		return err
	}
	...
	_, err = tx.Exec("DELETE FROM favorites WHERE movie_id = $1 AND user_id = $2", movieId, userId)
	if err != nil {
		return err
	}
